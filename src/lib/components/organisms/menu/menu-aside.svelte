...
<aside
  ...
  class={twMerge(
    "epic-transition bg-accent fixed top-0 left-0 z-20 h-dvh w-0 min-w-0 overflow-hidden pt-10 *:px-4",
    isOpen && "min-w-[70%]"
  )}
>
  {#if profile}
    <header class="border-secondary flex items-center gap-4 border-b pb-4">
      <CyberImage
    ...
    <ul class="py-4">
      {#each ENTRIES as entry (entry.href)}
        ...
        <li onclick={closeMenu} class="text-h-4 flex items-center gap-2">
...
<div
  ...
  class={twMerge(
    ...
    isOpen && "z-0 opacity-50",
    !isOpen && "-z-10 opacity-0"
  )}
></div>
